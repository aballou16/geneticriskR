...
#kfold
k <- 5 #number of fold
out <- "hypertension_k_fold" #output file prefix
pheno <- phenotype 
score <- score_col
...
for(i in 1:k){
  ...
  glm.obj <- glm(get(pheno) ~ get(score), data = train_data, family = "binomial")
...
for(i in 1:k){
  ...
  if (i==1){				  
    roc(testData[[pheno]], test_preds, plot=TRUE, ci=TRUE, col=colors[i], print.auc=TRUE, print.auc.col=colors[i], print.auc.y=0.5-(i*0.05))
  } else {
